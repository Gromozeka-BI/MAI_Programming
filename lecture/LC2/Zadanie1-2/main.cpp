...
public:
    ...
    // ћетоды, реализуемые по вариантам
    void add(FooString str);               // ¬ариант 4 Ц перевыделить пам€ть и добавить строку str в конец
};*/
...
public:
    ...
    FooString(char* tbuf) {
        int i = 0;
        while (tbuf[i] != 0) // ищем конец строки
            i++;
        val = i;
        buf = new char[val];
        buf = tbuf;
    ...
    void add(FooString& str) {
        int valCopy = getVal() + str.getVal(); // суммарна€ длина двух слов
        char *bufCopy = new char[valCopy];
        for (int i = 0; i < getVal(); i++) { //копируем изначальное слово кроме символа /0
            bufCopy[i] = outputMasI(i);
        }
        for (int i = getVal(); i <= valCopy; i++) { //копируем второе слово и символ /0
